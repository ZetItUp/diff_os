...
static inline void log_append(unsigned char attrib,char c)
{
    ...
    if (s_log_count<CONSOLE_LOG_CAP){s_log_count++;}
}

static inline int at_floor(void)
{
    if (!floor_enabled){return 0;}
    if (cursor_y<floor_y){return 1;}
    if (cursor_y>floor_y){return 0;}
    return cursor_x<=floor_x;
}

uint8_t vga_cell_height(void)
{
    if (s_vbe_console_active){return 16;}
    outb(0x3D4,0x09);
...
{
    if (s_vbe_console_active){return;}
    outb(0x3D4,0x0A);
...
void putch(char c)
{
    ...
    if (eflags&(1u<<9)){asm volatile("sti");}
}
...
void putch_color(unsigned char attrib,char c)
{
    ...
    {
        if (attrib!=current_attrib){current_attrib=attrib;}
        vbe_apply_colors_from_attrib(current_attrib);
        if (c=='\b')
        {
            if (!at_floor())
...
{
    current_attrib=attrib;
    if (s_vbe_console_active){vbe_apply_colors_from_attrib(current_attrib);}
}
...
{
    if (s_vbe_console_active){return (unsigned short)(cursor_y*SCREEN_WIDTH+cursor_x);}
