...
static void canon_exl_name(const char *in, char *out, size_t out_sz) {
    if (!in || !*in) { snprintf(out, out_sz, "diffc.exl"); return; }
    const char *base = in;
...
// Check if an EXL is currently in loading state
...
// Push EXL name onto the loading stack
                           ...
                           const char *self_name)
{
    ...
    for (uint32_t i = 0; i < hdr->import_table_count; ++i) {
        ...
        if (!p && is_loading(exl)) {
            // Avoid recursion; still try local resolution
            p = resolve_local_symbol(hdr, symtab, strtab, image, sym);
        }

        if (!p) p = resolve_exl_symbol(exl, sym);
    ...
    // Relocs — make pages RW before, RX after (enforced in load_exl)
    for (uint32_t i = 0; i < hdr->reloc_table_count; ++i) {
        uint32_t off = rel[i].reloc_offset;
        uint32_t idx = rel[i].symbol_index; // ABS32/PC32 use index, RELATIVE ignores
        ...
        switch (typ) {
            case DEX_ABS32:
                if (idx >= hdr->import_table_count) { printf("-> idx OOR\n"); return -6; }
...
const exl_t* load_exl(const FileTable *ft, const char *exl_name)
{
    ...
    if (rbytes < 0) {
        ...
        kfree(filebuf); pop_loading(tmp_name); return NULL;
    }
...
#ifdef DIFF_DEBUG
    // Verify the area is RWU before relocations
    ...
    // Lock .text to RX; mark everything USER (helpers flush TLB)
    ...
    if (hdr->symbol_table_count) {
        ...
        if (!symtab_copy) { printf("[EXL] symtab alloc fail\n"); kfree(filebuf); pop_loading(tmp_name); return NULL; }
        memcpy(symtab_copy, (const void*)(filebuf + hdr->symbol_table_offset),
               hdr->symbol_table_count * sizeof(dex_symbol_t));
    }
    if (hdr->strtab_size) {
        strtab_copy = kmalloc(hdr->strtab_size);
        if (!strtab_copy) { if (symtab_copy) kfree(symtab_copy); printf("[EXL] strtab alloc fail\n"); kfree(filebuf); pop_loading(tmp_name); return NULL; }
        memcpy(strtab_copy, (const void*)(filebuf + hdr->strtab_offset),
               hdr->strtab_size);
    }
    ...
    lib->header       = NULL;           // we do not keep the file header in RAM
...
#ifdef DIFF_DEBUG
    // Extra: dump first instruction word at entry for debugging
