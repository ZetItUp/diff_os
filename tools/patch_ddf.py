...
def is_reloc_section(s):
    name = s.name or ""
    # i386 typically uses SHT_REL (.rel.*) 
    # Be permissive but only accept .rel.*
...
def main():
    ...
    with open(inp, 'rb') as f:
        ...
        # Layout: header(80) + .text + .rodata + .data + symtab + strtab + relocs + .bss (zero filled)
        cur = 80
        ...
        # Create output with header space and copy raw section payloads
        ...
        # Loop through all .rel.* sections and translate each relocation
        for relsec in elf.iter_sections():
            ...
            for rel in relsec.iter_relocations():
                ...
                if rtype == ENUM_RELOC_TYPE_i386['R_386_PC32']:
                    ...
                    # Unknown/unsupported relocation type, skip it
                    continue
        ...
        hdr = struct.pack(
            ...
            1, 
            0,                                  # version major/minor
            (reloc_off if reloc_cnt else 0),    # reloc offset
            reloc_cnt,                          # reloc count
            t_off, t_sz,                        # .text
            r_off, r_sz,                        # .rodata
            d_off, d_sz,                        # .data
