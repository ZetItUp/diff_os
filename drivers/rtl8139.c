...
#define REG_TXADDR              0x20    // TX Buffer Address 0
                                        // 4 in total:
                                        // - 0x20
                                        // - 0x24
                                        // - 0x28
...
#define REG_BCMR                0x62    // Basic Mode Constrol Register (Physical)
...
static int alloc_dma_buffers(void)
{
    ...
    {
        tx_buffer_phys[i] = kernel_alloc_phys_page();
...
static void free_dma_buffers(void)
{
    ...
    kernel->printf("[DRIVER] RTL8139 - DMG Buffers freed!\n");
}
...
static void init_rx(void)
{
    ...
    kernel->outl(io_base + REG_RECR, rcr);
...
// - Poll until the child clears the bit
// - All registers returns to default values
...
void ddf_driver_init(kernel_exports_t *exports)
{
    ...
    // Attempt to get BAR0 for the device
