...
OFF_ENTRY_OFF   = 0x0C
OFF_TEXT_OFF    = 0x10; OFF_TEXT_LEN   = 0x14
OFF_RO_OFF      = 0x18; OFF_RO_LEN     = 0x1C
OFF_DATA_OFF    = 0x20; OFF_DATA_LEN   = 0x24
OFF_BSS_LEN     = 0x28
OFF_IMPORT_OFF  = 0x2C; OFF_IMPORT_CNT = 0x30
OFF_RELOC_OFF   = 0x34; OFF_RELOC_CNT  = 0x38
OFF_SYMTAB_OFF  = 0x3C; OFF_SYMTAB_CNT = 0x40
OFF_STRTAB_OFF  = 0x44; OFF_STRTAB_LEN = 0x48
...
def read_hdr(f):
    hdr = f.read(HDR_SIZE)
    if len(hdr) != HDR_SIZE:
        raise ValueError("filen är kortare än headern (0x100)")
    if u32(hdr, 0x00) != DEX_MAGIC:
        raise ValueError("inte en DEX/EXL (fel magic)")
    return hdr
...
def list_imports(f, hdr, strtab):
    imp_off = u32(hdr, OFF_IMPORT_OFF); imp_cnt = u32(hdr, OFF_IMPORT_CNT)
    ...
    for i in range(imp_cnt):
        raw = f.read(16)
        if len(raw) != 16:
            print(f"!! avhuggen importtabell vid post {i}", file=sys.stderr); break
...
def list_exports(f, hdr, strtab):
    sym_off = u32(hdr, OFF_SYMTAB_OFF); sym_cnt = u32(hdr, OFF_SYMTAB_CNT)
    ...
    for i in range(sym_cnt):
        raw = f.read(12)
        if len(raw) != 12:
            print(f"!! avhuggen symtab vid post {i}", file=sys.stderr); break
...
def main():
    ap = argparse.ArgumentParser(description="Lista imports och exports i en EXL/DEX")
    ap.add_argument("path", help="Sökväg till .exl/.dex")
    args = ap.parse_args()

    with open(args.path, "rb") as f:
        hdr = read_hdr(f)
        strtab_off = u32(hdr, OFF_STRTAB_OFF); strtab_len = u32(hdr, OFF_STRTAB_LEN)
