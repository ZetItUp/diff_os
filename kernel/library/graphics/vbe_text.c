...
// Physical location for classic VGA plane memory containing 8x16 ROM font.
#define VGA_FONT_PHYS_BASE  0x000A0000u
#define VGA_FONT_MAP_BYTES  0x2000u     // One page is enough, but keep headroom.

// Forward declarations for the kmap wrappers that exist in paging.c.
extern void* paging_kmap_phys(uint32_t phys, int slot);
extern void  paging_kunmap_phys(int slot);

static uint8_t g_vga_rom_ascii[96][16];
static int g_vga_rom_have = 0;

static int s_pt_pool_committed = 0;
 ...
 * We kmap the physical VGA plane at 0xA0000 instead of dereferencing a raw VA.
 */
static int vga_dump_font_ascii_8x16_direct(uint8_t out96x16[96][16])
{
    ...
    volatile const uint8_t *vram = (volatile const uint8_t*)paging_kmap_phys(VGA_FONT_PHYS_BASE, 0);
    ...
    // Try to detect stride (some BIOSes store 32 bytes/char, others 16).
    int stride = 32;
...
static inline int fb_is_mapped(void)
{
    ...
    return page_is_present((uint32_t)(uintptr_t)g_vbe.frame_buffer);
}
...
static void scroll_if_needed(void)
{
    ...
    if (!g_vbe.frame_buffer || g_vbe.bpp != 32 || !fb_is_mapped())
    {
        return;
    }
    ...
    // Manual memmove within the mapped framebuffer.
    for (uint32_t i = 0; i < total_bytes - shift_bytes; i++)
    {
        base[i] = base[i + shift_bytes];
    }
...
static inline void putpixel(uint32_t x, uint32_t y, uint32_t packed_px)
{
    ...
    if (x >= g_vbe.width || y >= g_vbe.height || x >= stride32)
    {
        return;
    }
...
static inline const uint8_t (*ensure_font(void))[16]
{
    if (s_font)
    {
        return s_font;
    ...
    if (g_vga_rom_have || vga_capture_rom_font_early())
    {
...
static void draw_glyph(char c, uint32_t x, uint32_t y)
{
    ...
    const uint8_t (*font)[16] = ensure_font();
...
{
    if (!g_vbe.frame_buffer || g_vbe.pitch == 0 || g_vbe.width == 0 || g_vbe.height == 0)
    {
        return 0;
    }
    ...
    // Always make sure we have a valid font buffer in kernel memory.
    if (g_vga_rom_have || vga_capture_rom_font_early())
    {
        s_font = (const uint8_t (*)[16])g_vga_rom_ascii;
    }
    ...
    if(!s_pt_pool_committed && fb_is_mapped())
    {
        paging_pt_pool_commit();
        s_pt_pool_committed = 1;
    }
...
{
    if (!g_vbe.frame_buffer || g_vbe.bpp != 32 || s_cols == 0 || s_rows == 0 || !fb_is_mapped())
    {
        return;
    }
